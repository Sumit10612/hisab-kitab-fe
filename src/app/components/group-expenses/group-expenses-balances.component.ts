	...
	template: `
		@if($group(); as group) {
			<app-layout headerHeight="160px">
				<div section="header">
					<app-group-expenses-header [group]="group" page="balance"></app-group-expenses-header>
				</div>
				<div section="detail" class="detail-section">
					@for (payment of settleExpenses(group); track $index) {
						<div class="balance-info">
							<span>{{group.members[payment.from].name}} owe {{group.members[payment.to].name}}</span>
