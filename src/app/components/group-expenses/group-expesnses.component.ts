...
@Component({
    ...
    template: `
        @if ($group(); as group) {
            <app-layout
                [headerHeight]="'160px'"
                (triggerOnScroll)="onScroll($event)"
            >
                <div section="header">
                ...
                <div section="detail">
                    @for (
                        kvp of $expensesByMonth() | keyvalue: noSort;
                        track kvp.key
                    ) {
                        <div class="expense-record-container">
                            <div class="month-group">
                                {{ kvp.key | date: "MMMM yyyy" | uppercase }}
                            </div>
                            @for (expense of kvp.value; track expense.id) {
                                ...
                                >
                                    <span class="expense-date">
                                        <span class="expense-date-month">{{
                                            expense.expenseDate
                                                | date: "MMM"
                                                | uppercase
                                        }}</span>
                                        <span class="expense-date-date">{{
                                            expense.expenseDate | date: "dd"
                                        }}</span>
                                    </span>
                                    <span class="emojis">{{
                                        getCategoryIcon(expense?.category ?? 0)
                                    }}</span>
                                    <span class="expense-desc">
                                        <span>{{ expense.description }}</span>
                                        @if (expense.where) {
                                            <span class="expense-desc-where"
                                                >at {{ expense.where }}</span
                                            >
                                        }
                                    ...
                                    <span class="expense-amount">
                                        <span class="expense-amount-paid-by"
                                            >{{
...
export class GroupExpensesComponent implements OnInit {
    ...
    private readonly $expenses = this.store.selectSignal(
    ...
    protected readonly $expensesByMonth = computed(() => {
        ...
        return mapValues(
            groupBy(this.$expenses(), (e) =>
                DateUtilities.yearMonth(e.expenseDate),
            ),
            (es) =>
                es.map((e) => ({
                    ...e,
