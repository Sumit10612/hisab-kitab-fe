...
export class ExpenseService {
    ...
    update(groupId: string, id: string, updateExpense: Expense): Promise<void> {
        ...
        return runTransaction(this.firestore, async (transaction) => {
            const { groupDoc, expenseDoc } = await this.getGroupExpense(
                transaction,
                groupId,
                id,
            );
            ...
            if (
                expenseDoc.amount !== updateExpense.amount ||
                oldKey !== newKey
            ) {
                const groupTotal =
                    +groupDoc.groupTotal -
            ...
            transaction.set(ref, toFirestoreModel(updateExpense), {
                merge: true,
            });
        });
    }
    ...
    private async getGroupExpense(
        ...
        const expenseDoc = throwIfNotFound(
            expenseSnapshot,
        ).data() as FirestoreExpense;
