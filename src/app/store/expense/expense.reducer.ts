...
export const expenseAdapter = createEntityAdapter<Expense>({
	...
	sortComparer: (e1, e2) => e1.expenseDate > e2.expenseDate ? -1 : e1.expenseDate < e2.expenseDate ? 1 : 0
});
...
export const expenseReducer = createReducer<ExpenseState>(
	...
	on(ExpenseAction.updateSuccess, (state, { expense }) => ({
		...state,
		expenses: expenseAdapter.updateOne({ id: expense.id, changes: expense }, state.expenses)
	})),
