...
export class GroupEffects {
                ...
                this.groupService.query$(userId).pipe(
                    map((changeDoc) => {
                        const members = Object.fromEntries(
                            Object.entries(changeDoc.group.members).map(
                                ([id, member]) => [
                                    id,
                                    {
                                        ...member,
                        ...
                        const group = {
                            ...
                            monthTotal: Object.fromEntries(
                                Object.entries(changeDoc.group.monthTotal).map(
                                    ([key, value]) => [key, round(value, 2)],
                                ),
                            ),
                            members,
                            currentMember: members[userId],
                            isCurrentMemberIsAdmin:
                                members[userId].role === MemberRole.admin,
                            activeMembers: values(
                                pick(members, changeDoc.group.memberIds),
                            ),
                        } as GroupInfo;

                        if (group.groupType === GroupType.SpiltExpense) {
                            const you = values(group.members).find(
                                (member) => member.id === userId,
                            );
                            group.groupTotal = round(
                                (you?.paid ?? 0) - (you?.share ?? 0),
                            );
                        }

                        switch (changeDoc.type) {
                            case "added":
                                return GroupAction.added({ group });
                            case "modified":
                                return GroupAction.modified({ group });
                            case "removed":
                                return GroupAction.removed({ id: group.id });
                        }
                    }),
                ),
            ),
    ...
    addMember$ = createEffect(() => {
        return this.action$.pipe(
            ...
            switchMap(async ({ code }) => {
                this.notification.showLoading();
                try {
                    await this.groupService.addMemeberToGroupViaCode(
                        this.$user().uid,
                        this.$user().name ?? "",
                        code,
                    );
    ...
    navigateHome$ = createEffect(
        () => {
            return this.action$.pipe(
                ofType(
                    GroupAction.createSuccess,
                    GroupAction.deleteSuccess,
                    GroupAction.addMemberSuccess,
                ),
                ...
                try {
                    ...
                    if (error === ErrorCode.NO_OTHER_ADMIN_FOUND) {
                        this.notification.error(
                            "Cannot leave, you are the only admin here.",
                        );
                    }
        ...
        return this.action$.pipe(
            ...
            switchMap(
                async ({
                    ...
                    this.notification.showLoading();
                    try {
                        await this.groupService.addCategoryToGroup(
                            groupId,
                            subCategoryName,
                            icon,
                            categoryId,
